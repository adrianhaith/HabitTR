...
plot(1+.5*rand(1,24)-.25,habit_index_av(:,1),'.','color',d.color.habit.untrained       ,'markersize',15)
plot(2+.5*rand(1,24)-.25,habit_index_av(:,2),'.','color',d.color.habit.trained         ,'markersize',15)
plot(3+.5*rand(1,24)-.25,habit_index_av(:,3),'.','color',d.color.habit.extendedTrained ,'markersize',15)
...
errorbar(1:3,nanmean(habit_index_av),seNaN(habit_index_av),'.k','markersize',1);

igood = find(~isnan(habit_index_av(:,1)));
%plot([0.5 3.5],(nanmean(habit_index_av(:,1))+2*std(habit_index_av(igood,1)))*[1 1],'k--')
plot([0.5 3.5],[0 0],'k')
set(gca,'tickdir','out');
ylabel('Habit index');
xlabel('Condition')
