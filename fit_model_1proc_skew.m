...
%                   lambda        - skewness parameter for ex-Gaussian distribution
%                   ]
%
...
paramsInit = [.4 .05 .95 .2 .1];
LB = [0 .01 .5 LB_soft 1];
%PLB = [.2 .02 .2 .02 .6 .1 LB_soft];
UB = [.75 100 UB_soft .5 1000];
...
like_fun = @(params) habit_lik_U_skew(RT,response,params,Nprocesses);
...
model.presponse = getResponseProbs_U_skew(model.xplot,model.paramsOpt,Nprocesses)
