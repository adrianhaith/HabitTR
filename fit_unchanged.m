...
for s=1:24
    for c=1:3
        if(~isempty(data(s,c).RT))
            ...
            %model(s,c) = fit_model(data(s,c).RT,data(s,c).response,2,NaN*ones(1,7),0);
            %model_constrained(s,c) = fit_model(data(s,c).RT,data(s,c).response,2,[constrained_params 1]);
            model_constrained_simp(s,c) = fit_model(data(s,c).RT,data(s,c).response,2,[constrained_params 1],0);
            model_constrained_simp_flex(s,c) = fit_model(data(s,c).RT,data(s,c).response,2,[constrained_params NaN],0);
        end
    end
end
...
for c=1:3
    for s=1:24
        if(~isempty(data(s,c).RT))
            rho_unconstrained(s,c) = model_constrained_simp_flex(s,c).paramsOpt(end);
            rho_constrained(s,c) = model_constrained_simp(s,c).paramsOpt(end);
        else
